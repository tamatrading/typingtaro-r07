...
const KeyboardHands: React.FC<Props> = ({ highlightedKey, currentInput }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 flex justify-center space-x-32 opacity-90 pointer-events-none select-none">
      {/* Left Hand */}
      <div className="flex space-x-4 items-end">
        {[3, 2, 1, 0].map((finger) => (
          <div
            key={`left-${finger}`}
            className={`w-8 ${
              finger === 1 ? 'h-[7.2rem]' :  // 中指は20%長く
              finger === 3 ? 'h-[4.8rem]' :  // 小指は20%短く
              'h-24'
              ...
              fingerPositions.some(pos => pos.hand === 'left' && pos.finger === finger)
                ? 'bg-blue-500 shadow-lg shadow-blue-500/50 animate-[pulse_1s_ease-in-out_infinite] ring-4 ring-blue-300/50'
                : 'bg-gray-300/90'
            } hover:scale-105`}
          />
        ))}
